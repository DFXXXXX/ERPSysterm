...
namespace ERPSysterm.MiddleWare
{
    public class CustomExceptionMiddleWare 
    {
        ...
        /// <param name="ex"></param>
        /// <returns></returns>
        private ObjectResult GetApiResponse(Exception ex)
        {
            return new ObjectResult(new { code = 200, msg = "", result = ex.Message.ToString() });
        }
        ...
        private async Task JsonHandle(HttpContext context, Exception ex)
        {
            ...
            var serialzeStr = JsonConvert.SerializeObject(apiResponse.Value);
            context.Response.ContentType = "application/json";
            await context.Response.WriteAsync(serialzeStr);
        }
